...
import { imprimirTicket } from "./ticketService";
...
const Ventas = () => {
  ...
  const [vistaActual, setVistaActual] = useState("productos"); // Nueva vista
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setProductosFiltrados(productosObtenidos); // Inicializa los productos filtrados
        setVentas(ventasObtenidas);
        setVentasFiltradas(ventasObtenidas); // Inicializa las ventas filtradas
  ...
  ...prev,
  {
    ...
    promocionNombre: promocion.nombre, // ← AGREGAMOS ESTO
  ...
  const agregarProductoAlCarrito = (productoId) => {
    ...
    const existe = productosSeleccionados.find((p) => p.productoId === productoId && !p.esPromocion);
  ...
  const modificarCantidadProducto = (productoId, nuevaCantidad) => {
    const productoSeleccionado = productosSeleccionados.find(
      (p) => p.productoId === productoId && !p.esPromocion
    );
    ...
    setProductosSeleccionados((prev) =>
      prev.map((p) =>
        p.productoId === productoId && !p.esPromocion
          ? { ...p, cantidad: nuevaCantidad, subtotal: nuevaCantidad * p.precio }
          : p
      )
    );
...
const ventaData = {
  ...
  promociones: promocionesSeleccionadas.map(p => ({
    ...
    nombre: p.promocionNombre || p.descripcion, // Usar promocionNombre si está disponible
    precio: p.precio,
    ...
    console.log('Datos enviados al backend:', ventaData); // Para debug
    ...
    // Recargar productos para obtener el stock actualizado desde el servidor
    ...
    let mensajeError = "No se pudo realizar la venta.";
    ...
    <div className="ventas-container">
      ...
      <div className="form-group">
        ...
        <select value={metodoPago} onChange={(e) => setMetodoPago(e.target.value)}>
          ...
          <option value="mercadopago_qr">MercadoPago (QR)</option>
      ...
      {vistaActual === "productos" ? (
        ...
        <>
          <h4 className="section-title">
            <FaTags /> Promociones disponibles
          </h4>
          ...
          <div className="promociones-list">
            {promociones.length > 0 ? (
              promociones.map((promocion) => {
                ...
                switch (promocion.tipo) {
                  case "2x1":
                    stockDisponible = producto1?.stock >= 2 && (!producto2 || producto2.stock >= 1);
                    mensajeStock = `Stock: ${producto1?.stock || 0}${producto2 ? ` / ${producto2.stock}` : ''}`;
                    break;
                  case "Descuento por volumen":
                    stockDisponible = producto1?.stock >= promocion.cantidadMinima;
                    mensajeStock = `Stock: ${producto1?.stock || 0} (Min: ${promocion.cantidadMinima})`;
                    break;
                  case "Pack combo":
                    stockDisponible = producto1?.stock >= 1 && producto2?.stock >= 1;
                    mensajeStock = `Stock: ${producto1?.stock || 0} / ${producto2?.stock || 0}`;
                    break;
                  case "Descuento porcentaje":
                    stockDisponible = producto1?.stock >= 1;
                    mensajeStock = `Stock: ${producto1?.stock || 0}`;
                    break;
                  default:
                    stockDisponible = false;
                    mensajeStock = "Stock no disponible";
                }
                ...
                return (
                  <div key={promocion._id} className="promocion-card">
                    ...
                    <div className="promocion-info">
                      <h5 className="promocion-nombre">{promocion.nombre}</h5>
                      <p className="promocion-tipo">Tipo: {promocion.tipo}</p>
                      <p className="promocion-precio">Precio: ${promocion.precio}</p>
                      <p className="promocion-productos">
                        {producto1?.descripcion}
                        {producto2 && ` + ${producto2.descripcion}`}
                      </p>
                      <p className={`promocion-stock ${!stockDisponible ? 'sin-stock' : ''}`}>
                        {mensajeStock}
                      </p>
  ...
  {/* Promociones */}
  {venta.promociones && venta.promociones.map((promo, i) => (
    <div key={`promo-${i}`} style={{color: '#28a745', fontWeight: 'bold'}}>
      🏷️ {promo.nombre} - Cantidad: {promo.cantidad} - Subtotal: ${promo.subtotal}
    </div>
  ))}
</td>
                  ...
                  <td>
                    <div className="acciones-container">
                      ...
                      <button
                        ...
                        onClick={() => eliminarUnaVenta(venta._id)}
              ...
              <tr>
                <td colSpan="6" className="no-ventas">No se encontraron ventas en el período seleccionado.</td>
              </tr>
            )}
          </tbody>
        </table>
